...
const regionalColleges = [
  ...
  // Andhra Pradesh - Degree Colleges & Universities
  ...
  // Telangana - Engineering Colleges & Universities
...
export async function GET() {
  try {
    ...
    for (const collegeName of regionalColleges) {
      // Generate a simple code
      const code = collegeName.split(' ').map(w => w[0]).join('').substring(0, 4).toUpperCase() + Math.floor(Math.random() * 10000);
      ...
      if (!existing) {
        const college = await prisma.college.create({
          data: {
            name: collegeName,
            code: code,
            description: `A reputed institution in Andhra Pradesh / Telangana.`,
          }
        });
    ...
    return NextResponse.json({ message: `Successfully added ${count} new colleges from AP and Telangana!` });
  } catch (error: any) {
    console.error(error);
